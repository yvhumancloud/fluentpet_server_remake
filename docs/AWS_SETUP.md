...
| `/fluentpet/prod/ANTHROPIC_API_KEY` | from console.anthropic.com; omit or a single space = AI endpoints answer 503. Optional `/fluentpet/prod/AI_MODEL` (default `claude-opus-5`). For a gateway instead: `ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN` (see `.env.example`) |
